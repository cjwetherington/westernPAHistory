		...
		{foreach key=field item=navigationMenuItemAssignment from=$navigationMenu->menuTree}
			...
			<li class="{$liClass|escape} {$navigationMenuItemAssignment->navigationMenuItem->getType()|lower}{if $hasChildren} dropdown{/if}">
				<a href="{$navigationMenuItemAssignment->navigationMenuItem->getUrl()}"
					...
					{if $hasChildren}
						id="{$id|escape}-{'navMenuDropdown'|concat:$field}"
				...
				{if $hasChildren}
					<div class="dropdown-menu{if $id === 'userNav'} dropdown-menu-right{/if}" aria-labelledby="{$id|escape}-{'navMenuDropdown'|concat:$field}">
